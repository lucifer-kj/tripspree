...
export function TripHeroCard() {
  ...
  return (
    <div className="relative w-full aspect-[4/3] sm:aspect-[16/11] lg:aspect-[4/3] rounded-[28px] overflow-hidden border border-[#25183e] bg-[#130c24] shadow-2xl group select-none">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#0c0717]/95 via-[#0c0717]/35 to-transparent" />
      ...
      <div className="absolute top-5 sm:top-6 left-5 sm:left-6 right-5 sm:right-6 flex items-center justify-between z-10">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20 backdrop-blur-md border border-white/25 text-white hover:bg-white/35 transition-all cursor-pointer shadow-md"
          >
      ...
      <div className="absolute left-5 sm:left-6 top-20 sm:top-24 flex flex-col gap-2.5 z-10">
        {activeSlide.thumbnails.map((thumb, idx) => (
          <div
            ...
            className={`relative h-11 w-11 sm:h-12 sm:w-12 rounded-xl overflow-hidden border shadow-lg transition-transform duration-200 hover:scale-105 cursor-pointer ${
              idx === currentIndex
      ...
      <div className="absolute bottom-5 sm:bottom-7 left-5 sm:left-7 right-5 sm:right-7 z-10 space-y-1.5">
        <h2 className="font-serif text-2xl sm:text-3xl lg:text-4xl font-normal tracking-tight text-white drop-shadow-md">
          {activeSlide.name}
        </h2>
